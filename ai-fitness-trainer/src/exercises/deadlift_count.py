...
def process_video(stop_signal, count, count2):
    ...
    while not stop_signal.value:
        ...
        if len(lmList) != 0:
            ...
            per2 = np.interp(angle1, (90, 170), (100, 0))
            ...
            # Thanh tiến trình cho chân trái (bên trái màn hình)
            cv2.rectangle(img, (50, 100), (100, 400), (255, 0, 0), 3)
            cv2.rectangle(img, (50, int(400 - (per * 3))), (100, 400), (255, 0, 0), cv2.FILLED)
            cv2.putText(img, f'{int(per)}%', (50, 90), cv2.FONT_HERSHEY_PLAIN, 2, (255, 0, 0), 2)

            # Thanh tiến trình cho chân phải (bên phải màn hình)
            cv2.rectangle(img, (1180, 100), (1230, 400), (255, 0, 0), 3)
            cv2.rectangle(img, (1180, int(400 - (per2 * 3))), (1230, 400), (255, 0, 0), cv2.FILLED)
            cv2.putText(img, f'{int(per2)}%', (1180, 90), cv2.FONT_HERSHEY_PLAIN, 2, (255, 0, 0), 2)

            # Hiển thị số lần thực hiện
            cv2.putText(img, f'Left Reps: {int(count.value)}', (50, 50), cv2.FONT_HERSHEY_PLAIN, 2, (255, 0, 0), 2)
            cv2.putText(img, f'Right Reps: {int(count2.value)}', (1000, 50), cv2.FONT_HERSHEY_PLAIN, 2, (255, 0, 0), 2)
...
def save_to_database(count, count2):
    try:
        ...
        response = requests.post('http://localhost:5000/save_deadlift_data', json=data, headers=headers)
        if response.ok:
